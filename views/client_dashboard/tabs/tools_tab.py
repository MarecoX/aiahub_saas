...
def _render_business_hours_section(t_config: dict) -> dict:
    """Renderiza a seção de Horário de Atendimento com toggles por dia."""
    ...
    if bh_active:
        st.info(
            "Fora do horário configurado, a IA não responderá (ou enviará a mensagem personalizada abaixo)."
        )
        ...
        off_message = st.text_area(
            ...
            value=bh.get("off_message", ""),
            height=80,
            placeholder="Ex: Nosso horário de atendimento é de segunda a sexta, das 8h às 18h. Retornaremos em breve!",
            help="Se preenchida, essa mensagem será enviada quando alguém mandar mensagem fora do horário.",
            key="bh_off_message",
        )
    else:
...
def render_tools_tab(user_data):
    ...
    bh_config = _render_business_hours_section(t_config)
    st.divider()
    ...
    if st.button("Salvar Integracoes"):
        ...
        # Save business hours
        new_tools_config["business_hours"] = bh_config
