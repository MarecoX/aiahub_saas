...
from scripts.saas_db import get_connection, clear_chat_history
...
def render_admin_view():
        ...
        try:
            with get_connection() as conn:
                 with conn.cursor() as cur:
                    cur.execute("SELECT id, name, token, username, is_admin, gemini_store_id, tools_config, human_attendant_timeout, api_url, created_at FROM clients ORDER BY created_at DESC")
                    rows = cur.fetchall()
                    if rows:
    ...
    def create_client(name, token, prompt, username, password, api_url=None, timeout=60):
        from scripts.auth_manager import hash_password
        try:
            ...
            with get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(sql, (name, token, prompt, store_id, tools_config, timeout, data_url, username, pwd_hash))
    ...
    def update_config(c_id, config_str, timeout, url):
        try:
            with get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute("UPDATE clients SET tools_config = %s, human_attendant_timeout = %s, api_url = %s WHERE id = %s", 
    ...
    # --- UI ---
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["➕ Criar Cliente", "📋 Lista Geral", "🔐 Acessos", "⚙️ Config Avançada", "🛠️ Debug"])
    ...
    with tab1:
        ...
        with c2:
            prompt = st.text_area("Prompt Inicial", height=100, value="Assistente útil.")
    ...
    with tab3:
        ...
        st.caption("Use isso para definir acessos de clientes antigos (ex: Algar) ou resetar senhas.")
        
        df = list_clients()
        if not df.empty:
            sel_cli = st.selectbox("Selecione o Cliente para editar:", df['name'].tolist(), key="sel_access")
            row_cli = df[df['name'] == sel_cli].iloc[0]
            
            st.info(f"Usuário Atual: {row_cli['username'] if 'username' in row_cli and row_cli['username'] else 'Sem usuário definido'}")
            
            c_new_user = st.text_input("Novo Username (Opcional)", placeholder="Deixe vazio para manter")
            c_new_pass = st.text_input("Nova Senha (Opcional)", type="password", placeholder="Deixe vazio para manter")
    ...
    with tab4:
        ...
        df = list_clients()
        if not df.empty:
            sel = st.selectbox("Cliente", df['name'].tolist(), key="sel_cfg") # Unique key
            row = df[df['name'] == sel].iloc[0]
            
            cfg_txt = st.text_area("JSON Tools", value=json.dumps(row.get('tools_config', {}), indent=2))
            to_val = st.number_input("Timeout", value=row.get('human_attendant_timeout', 60))
